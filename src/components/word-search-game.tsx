...
export default function WordSearchGame() {
  ...
  const [foundWords, setFoundWords] = useState<{ word: string; cells: [number, number][]; color: string }[]>([])
  const [score, setScore] = useState(0)
  const [totalScore, setTotalScore] = useState(0)
  const [selectedCells, setSelectedCells] = useState<[number, number][]>([])
  const [isSelecting, setIsSelecting] = useState(false)
  const [gameState, setGameState] = useState<'ready' | 'playing' | 'won' | 'completed'>('ready')
  ...
  const generateMatrix = (words: string[], size: number): string[][] => {
    ...
    for (let i = 0; i < size; i++) {
      for (let j = 0; j < size; j++) {
        if (matrix[i][j] === '') {
          matrix[i][j] = chineseChars[Math.floor(Math.random() * chineseChars.length)]
        }
      }
    }
  ...
  const canPlaceWord = (matrix: string[][], word: string, row: number, col: number, direction: string, size: number): boolean => {
  ...
  const placeWord = (matrix: string[][], word: string, row: number, col: number, direction: string) => {
    for (let i = 0; i < word.length; i++) {
      const currentRow = direction === 'horizontal' ? row : row + i
      const currentCol = direction === 'horizontal' ? col + i : col
  ...
  return (
    <Card className="w-full max-w-4xl mx-auto bg-gradient-to-br from-purple-500 to-pink-500 text-white">
      <CardHeader className="text-center">
        ...
        <p className="text-lg">关卡：{currentLevel + 1} / {LEVELS.length}</p>
      </CardHeader>
      <CardContent>
        <div className="grid gap-6">
                  ...
                  row.map((cell, colIndex) => (
                    <motion.div
                      ...
                      className={`w-12 h-12 flex items-center justify-center border-2 rounded-md font-bold text-2xl cursor-pointer transition-all duration-300
                        ${isCellSelected(rowIndex, colIndex) ? 'bg-yellow-300 text-purple-800' :
                          isCellFound(rowIndex, colIndex) ? 'bg-white text-purple-900 shadow-lg scale-105' : 'bg-white/20'}`}
                      ...
                      style={{
                        color: isCellFound(rowIndex, colIndex) ? getCellColor(rowIndex, colIndex) : undefined,
                        textShadow: isCellFound(rowIndex, colIndex) ? '0 0 10px rgba(255,255,255,0.5)' : 'none'
                      }}
                    >
                      {cell}
                    </motion.div>
          ...
          {gameState === 'ready' && (
            <Button onClick={() => startLevel(currentLevel)} size="lg" className="bg-green-500 hover:bg-green-600">
              开始游戏
            </Button>
          )}
          {gameState === 'playing' && (
            <Button onClick={() => startLevel(currentLevel)} size="lg" className="bg-yellow-500 hover:bg-yellow-600">
              重新开始本关
            </Button>
          )}
          {gameState === 'won' && (
            <div className="text-center">
              ...
              }} size="lg" className="bg-green-500 hover:bg-green-600">
